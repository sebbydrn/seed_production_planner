<script>
	function submit_prod_cost_sched() {
		swal({
		  	title: "Submit for Approval",
		  	text: "Do you want to submit this production cost schedule for approval?",
		  	icon: "warning",
		  	buttons: {
		  		cancel: "No",
		  		confirm: "Yes, Proceed",
		...
		.then((submit) => {
		  	if (submit) {
		  		...
		  		$.ajax({
		  			...
		  			success: (res) => {
		  				if (res == "success") {
		  					swal("Success! Pending approval of production cost schedule.", {
					      		icon: "success",
					   		}).then((result) => {
