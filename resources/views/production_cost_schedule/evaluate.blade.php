...
@section('pageContent')
				...
				<tbody>
					...
					@foreach($seed_cleaning as $item)
						...
						<tr>
							...
							<td><i>average 20-kg bags (clean seeds) per ha</i></td>
		...
		<div class="panel-body">
			<table class="table table-bordered" style="width: 100%;">
				...
				<tfoot>
					...
					<tr>
						<td></td>
						<td><i>Volume of clean seeds, FS-RS (kg)</i></td>
					...
					<tr>
						<td></td>
						<td><i>Volume of clean seeds, BS-FS (kg)</i></td>
	...
	<section class="panel">
		...
		<div class="panel-body">
			@if($production_cost_schedule->is_approved === 0 && $action == "Reviewing")
				{{-- Accountant --}}
				<form action="{{route('production_cost_schedule.submit_evaluation')}}" method="POST">
					<input type="hidden" name="_token" id="csrf-token" value="{{csrf_token()}}">

					<input type="hidden" name="production_cost_id" value="{{$production_cost_id}}">
					...
					<div class="row mt-lg">
						<div class="col-lg-6 col-md-12">
							...
							<select name="is_approved" id="is_approved" class="form-control">
								...
								<option value="3">Need Revision</option>
							</select>
						</div>
			...
			@elseif($production_cost_schedule->is_approved === 2 && $action == "Approving")
				{{-- BDD Head --}}
				<form action="{{route('production_cost_schedule.submit_evaluation')}}" method="POST">
					<input type="hidden" name="_token" id="csrf-token" value="{{csrf_token()}}">
			...
			@else
				<h3>You do not have access to evaluate or approve.</h3>
			@endif
		</div>
	</section>
@endsection