<script>
	function generatePlantingReport() {
		...
		if (station && year && sem && seedClass) {
			...
			$.ajax({
				type: 'POST',
				url: "{{route('planting_report.generate')}}",
				data: {
					'_token': "{{csrf_token()}}",
					'stationID': station,
					'year': year,
				...
				success: (report) => {
					...
					// Destroy planting report table datatable
					if ($.fn.DataTable.isDataTable('#plantingReportTable')) {
						$('#plantingReportTable').DataTable().destroy()
					}

					// Empty planting report tbody
					$('#plantingReportTable tbody').empty()

					// Initiate planting report table datatable
					let plantingReportTable = $('#plantingReportTable').DataTable({
						columns: [
							{
								'name': 'station',
								'data': 'station',
					...
					var breeder = report.filter(report => report.seedClass == "Nucleus");

					if (breeder.length) {
						// Add breeder seed production row header
						let breederRow = plantingReportTable
							.row.add({
						...
						breeder.map(currElement => {
							plantingReportTable.row.add({
								'station': currElement['station'],
								'variety': currElement['variety'],
								'source': currElement['source'],
								'source_lot': currElement['source_lot'],
								'source_lab': currElement['source_lab'],
					...
					let foundation = report.filter(report => report.seedClass == "Breeder")

					// If data has Foundation Seed Production
					if (foundation.length) {
						// Add foundation seed production row header
						let foundationRow = plantingReportTable
							.row.add({
					...
					let registered = report.filter(report => report.seedClass == "Foundation")

					// If data has Registered Seed Production
					if (registered.length) {
						// Add registered seed production row header
						let registeredRow = plantingReportTable
							.row.add({
					...
					// Draw table
					plantingReportTable.draw();

					// Display table
					document.getElementById('plantingReportTable').style.display = "table"
	...
	function exportToExcel() {
		...
		if (station && year && sem && seedClass) {
			...
			form.method = 'POST'
			form.action = "{{route('planting_report.export')}}"
	...
	function exportToPDF() {
		...
		if (station && year && sem && seedClass) {
			...
			let form = document.createElement('form')
			form.method = 'POST'
			form.action = "{{route('planting_report.exportToPDF')}}"
			form.target = "_blank"
