<script>
	...
	function viewFormInfo(pestManagementID) {
		// Form info
		$.ajax({
			type: 'GET',
			url: `{{url('seed_trace_geotag/pest_management')}}/`+pestManagementID,
			dataType: 'JSON',
			success: (res) => {
				...
				document.getElementById('pestType').innerHTML = res.pest_type
				document.getElementById('pestSpec').innerHTML = res.pest_spec
				...
				// Generate map
				document.getElementById('pestManagementFormMapContainer').innerHTML = '<div id="pestManagementFormMap" style="height: 400px;"></div>'
				...
				pestManagementFormMap.invalidateSize()
			}
		})
	}
	...
	function createPlotDetailsMap(coordinates) {
		...
		// Create map
		pestManagementFormMap = L.map('pestManagementFormMap')
		pestManagementFormMap.addLayer(GoogleMapsHybrid)

		let point = coordinates.split(",")
		let marker = new L.marker(L.latLng(parseFloat(point[0]), parseFloat(point[1]))).addTo(pestManagementFormMap)

		// Set map view
		pestManagementFormMap.setView(L.latLng(parseFloat(point[0]), parseFloat(point[1])), 18)
	}
