<script>
	...
	setInterval(function() {
		$.ajax({
			type: 'GET',
			url: "{{route('data_monitoring.show_data')}}",
			dataType: 'JSON',
			success: (res) => {
				console.log(res)
				...
				if (dailyData.length == 0) {
					...
					dailyData.forEach(function (item, index) {
						if (count <= 5) {
							dailyDataRows += '<p class="mt-0 mb-0"><span style="color: #28a745;">[ '+item.timestamp+' ]</span> Data: [ Year:"'+item.year+'";Semester:"'+item.sem+'";Variety:"'+item.variety+'";Seed Class:"'+item.seed_class+'";Station:"'+item.station+'" ]</p>'
						}
						count++;
					})
				}
