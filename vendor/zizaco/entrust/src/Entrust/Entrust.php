...
class Entrust
{
    ...
    {
        $this->app = $app;
        $this->system_id = Config::get('entrust.system_id');
    }
    ...
    {
        if ($user = $this->user()) {
            return $user->hasRole($role, $requireAll, $this->system_id);
        }
    ...
    {
        if ($user = $this->user()) {
            return $user->can($permission, $requireAll, $this->system_id);
        }
    ...
    {
        if ($user = $this->user()) {
            return $user->ability($roles, $permissions, $options, $this->system_id);
        }
    ...
    public function routeNeedsRole($route, $roles, $result = null, $requireAll = true)
    {
        ...
        $closure = function () use ($roles, $result, $requireAll) {
            $hasRole = $this->hasRole($roles, $requireAll, $this->system_id);
    ...
    public function routeNeedsPermission($route, $permissions, $result = null, $requireAll = true)
    {
        ...
        $closure = function () use ($permissions, $result, $requireAll) {
            $hasPerm = $this->can($permissions, $requireAll, $this->system_id);
    ...
    public function routeNeedsRoleOrPermission($route, $roles, $permissions, $result = null, $requireAll = false)
    {
        ...
        $closure = function () use ($roles, $permissions, $result, $requireAll) {
            $hasRole  = $this->hasRole($roles, $requireAll, $this->system_id);
            $hasPerms = $this->can($permissions, $requireAll, $this->system_id);
