...
class DiseaseManagementController extends Controller {
	...
	public function index() {
		...
		return view('seed_trace_geotag.disease_management.index', compact(['role']));
	}
	...
	public function create() {
		...
		return view('seed_trace_geotag.disease_management.create', compact(['role', 'philriceStationID', 'years']));
	}

	public function store(Request $request) {
		DB::beginTransaction();
		try {
			$data = new DiseaseManagement();
			$data->production_plot_code = $request->productionPlotCode;
			$data->crop_phase = $request->cropPhase;
			$data->disease_type = $request->disease;
			$data->other_disease = $request->otherDisease;
			...
			return redirect()->back()->with('success', 'Disease management form successfully added.');
		} catch (Exception $e) {
			DB::rollback();
            ...
            return redirect()->back()->with('error', 'Error adding disease management form.');
		}
	}

	public function show($id) {
        $diseaseManagement = DiseaseManagement::where('disease_management_id', $id)->first();
        echo json_encode($diseaseManagement);
    }

	public function datatable(Request $request) {
		$data = DiseaseManagement::get();
		$data = collect($data);
		...
		return Datatables::of($data)
			...
			->addColumn('actions', function($data) {
				$actions = "<button type='button' class='mb-xs mt-xs mr-xs btn btn-info' onclick='viewFormInfo(".$data->disease_management_id.")'><i class='fa fa-eye'></i> View</button>&nbsp;";
    ...
    public function activities(Request $request) {
    	...
    	$diseaseManagement = DiseaseManagement::select('crop_phase', 'datetime_start', 'datetime_end')
    											->where('production_plot_code', '=', $productionPlotCode)
    											->get();

    	$data = array();
    	if ($diseaseManagement) {
	    	foreach ($diseaseManagement as $item) {
	    		$activityName = "Disease Management (" . $item->crop_phase . ")";
