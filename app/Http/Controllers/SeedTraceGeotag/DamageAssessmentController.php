...
class DamageAssessmentController extends Controller {
	...
	public function index() {
		...
		return view('seed_trace_geotag.damage_assessment.index', compact(['role']));
	}
	...
	public function create() {
		...
		return view('seed_trace_geotag.damage_assessment.create', compact(['role', 'philriceStationID', 'years']));
	}

	public function store(Request $request) {
		DB::beginTransaction();
		try {
			$data = new DamageAssessment();
			$data->production_plot_code = $request->productionPlotCode;
			$data->timestamp = date('Y-m-d H:i', strtotime($request->date . ' ' . $request->time));
			$data->damage_cause = $request->damageCause;
			$data->damage_spec = $request->damageSpec;
			...
			return redirect()->back()->with('success', 'Damage Assessment form successfully added.');
		} catch (Exception $e) {
			DB::rollback();
            ...
            return redirect()->back()->with('error', 'Error adding damage assessment form.');
		}
	}

	public function show($id) {
        $damageAssessment = DamageAssessment::where('damage_assessment_id', $id)->first();
        echo json_encode($damageAssessment);
    }

	public function datatable(Request $request) {
		$data = DamageAssessment::get();
		$data = collect($data);
		...
		return Datatables::of($data)
			...
			->addColumn('actions', function($data) {
				$actions = "<button type='button' class='mb-xs mt-xs mr-xs btn btn-info' onclick='viewFormInfo(".$data->damage_assessment_id.")'><i class='fa fa-eye'></i> View</button>&nbsp;";
    ...
    public function activities(Request $request) {
    	...
    	$damageAssessment = DamageAssessment::select('timestamp')
