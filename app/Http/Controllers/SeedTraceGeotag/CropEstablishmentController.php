...
class CropEstablishmentController extends Controller {
	...
	public function index() {
		...
		return view('seed_trace_geotag.crop_establishment.index', compact(['role']));
	}
	...
	public function create() {
		...
		return view('seed_trace_geotag.crop_establishment.create', compact(['role', 'philriceStationID', 'years']));
	}

	public function store(Request $request) {
		DB::beginTransaction();
		try {
			$data = new CropEstablishment();
			$data->production_plot_code = $request->productionPlotCode;
			$data->activity = $request->activity;
			$data->transplanting_method = $request->transplantingMethod;
			...
			return redirect()->back()->with('success', 'Crop establishment form successfully added.');
		} catch (Exception $e) {
			DB::rollback();
            ...
            return redirect()->back()->with('error', 'Error adding crop establishment form.');
		}
	}

	public function show($id) {
		$cropEstablisment = CropEstablishment::where('crop_establishment_id', '=', $id)->first();
        echo json_encode($cropEstablisment);
	}

	public function datatable(Request $request) {
		$data = CropEstablishment::get();
		$data = collect($data);
		...
		return Datatables::of($data)
			...
			->addColumn('actions', function($data) {
				$actions = "<button type='button' class='mb-xs mt-xs mr-xs btn btn-info' onclick='viewFormInfo(".$data->crop_establishment_id.")'><i class='fa fa-eye'></i> View</button>&nbsp;";
    ...
    public function activities(Request $request) {
    	...
    	$cropEstablisment = CropEstablishment::select('activity', 'datetime_start', 'datetime_end')
    											->where('production_plot_code', '=', $productionPlotCode)
    											->get();

    	$data = array();
    	if ($cropEstablisment) {
	    	foreach ($cropEstablisment as $item) {
