...
class LandPreparationController extends Controller {
	...
	public function index() {
		...
		return view('seed_trace_geotag.land_preparation.index', compact(['role']));
	}
	...
	public function create() {
		...
		return view('seed_trace_geotag.land_preparation.create', compact(['role', 'philriceStationID', 'years']));
	}

	public function store(Request $request) {
		DB::beginTransaction();
		try {
			$data = new LandPreparation();
			...
			return redirect()->back()->with('success', 'Land preparation form successfully added.');
		} catch (Exception $e) {
			DB::rollback();
            ...
            return redirect()->back()->with('error', 'Error adding land preparation form.');
		}
	}

	public function show($id) {
		$landPreparation = LandPreparation::where('land_preparation_id', '=', $id)->first();
        echo json_encode($landPreparation);
	}

	public function datatable(Request $request) {
		$data = LandPreparation::get();
		$data = collect($data);
		...
		return Datatables::of($data)
			...
			->addColumn('actions', function($data) {
				$actions = "<button type='button' class='mb-xs mt-xs mr-xs btn btn-info' onclick='viewFormInfo(".$data->land_preparation_id.")'><i class='fa fa-eye'></i> View</button>&nbsp;";
    ...
    public function activities(Request $request) {
    	...
    	$landPreparation = LandPreparation::select('activity', 'crop_phase', 'datetime_start', 'datetime_end')
    	...
    	if ($landPreparation) {
	    	foreach ($landPreparation as $item) {
	    		$activityName = $item->activity . " (" . $item->crop_phase . ")";
